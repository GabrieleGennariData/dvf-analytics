...
| --- | --- | ---: | --- |
| staging | `stg_dvfplus__mutations` | 16 565 022 | lecture des 97 CSV départementaux (séparateur `\|`), typage. **Une ligne = une mutation**, le grain est fourni par la source |
| staging | `stg_cog__communes` | 37 496 | le Code officiel géographique 2026 de l'Insee - DVF+ ne livre aucun nom de commune |
| intermediate | `int_dvfplus__mutations` | 16 527 605 | vocabulaire canonique, `prix_m2`, drapeaux de population |
...
La source arrivant déjà à la maille mutation, ce pipeline n'a **ni déduplication ni changement de grain** : il reçoit le grain qu'il publie au lieu de le reconstruire, et l'unicité de `idmutation` est une propriété vérifiée, pas une hypothèse.

## Démarrage rapide

Prérequis : Python **3.14.7** (les épinglages de `requirements.txt` n'ont été résolus que sous cette version), **dbt Fusion** - un binaire autonome, pas un paquet pip ; version utilisée ici `2.0.0-preview.212` - et **py7zr** (`pip install py7zr`, hors `requirements.txt` qui ne décrit que l'application déployée). Prévoir **25 Go de disque libres** : 1,1 Go d'archive, 8,9 Go de CSV extraits, et la base de travail `data/dvf.duckdb` autour de **11 Go**, sa taille variant d'un build à l'autre.
...
**Maille mutation, pas maille local.** Une mutation est un acte : elle peut porter plusieurs biens et une seule `valeur_fonciere`. Descendre à la maille local obligerait à répartir un prix unique entre plusieurs biens selon une règle inventée ; la maille mutation ne répartit rien. DVF+ livre ce grain directement - une ligne = un acte, identifiant unique vérifié sur les 16 565 022 lignes.
...
**Une discontinuité de la source, déclarée parce qu'elle se voit.** La part des maisons vendues avec une dépendance enregistrée passe de ~5 % (2014-2019) à 9 % (2020) puis 30-37 % (2021-2025) : c'est un changement d'**enregistrement** chez l'éditeur, pas un fait de marché. Il ne contamine pas les mesures - la surface reste celle du logement dans les deux ères, et la médiane nationale Maison ne montre aucune rupture au passage (1 739 €/m² en 2014, 1 897 en 2020, 2 030 en 2021, 2 164 en 2025) - mais il change le sens de `n_locaux` selon l'année.
...
- **éligibles** - les mêmes 9 817 126 aujourd'hui : la typologie officielle garantit une surface positive sur tout le périmètre. La coïncidence est une propriété des données, pas de la construction - le garde `sbati > 0` reste, et le test d'identité des populations la surveille ;
...
Aucune ligne n'est supprimée à ce stade : ce sont des drapeaux. Et `prix_m2` n'est calculé **qu'une fois**, dans l'intermediate - jamais recalculé dans un mart ni dans l'application, pour qu'il n'existe pas deux définitions du même rapport.

**`nature_mutation = 'Vente'`, égalité stricte.** Un `like 'Vente%'` ferait entrer 1 168 083 mutations de plus : ventes en l'état futur d'achèvement (1 099 309) et terrains à bâtir (68 774). Aucune valeur ne sortirait des plages, aucun test ne se déclencherait : l'erreur serait invisible parce que plausible. L'exclusion de la VEFA est en plus **prouvée** par une colonne indépendante : la source porte un drapeau `vefa`, transporté jusqu'à `fct_mutations`, où `assert_vefa_exclue` vérifie qu'il est faux partout.

**La commune n'est définie que quand elle est unique.** Un acte peut toucher plusieurs communes : DVF+ livre la liste. Plutôt que d'attribuer arbitrairement l'acte à la première, `code_commune` reste absent quand `nbcomm > 1` (1,19 % des ventes, 180 390 actes) : ces actes comptent dans les chiffres départementaux mais pas dans la table commune, et l'écart entre les deux totaux est exactement leur compte - une identité testée, pas un résidu toléré.

**Le seuil de 30 ventes s'applique aux deux années.** Le classement par variation annuelle exige 30 ventes plausibles l'année courante **et** l'année précédente : 48 161 cellules candidates au lieu de 55 180. Les 7 019 écartées avaient une base sous le seuil l'année d'avant, avec un minimum observé d'**une seule vente** - une variation en pourcentage bâtie sur une médiane d'une observation reste plausible à l'œil et n'est pas une donnée.

**Les mutations rejetées sont mises de côté, pas détruites.** Les 37 417 mutations à valeur foncière inexploitable vivent dans `int_dvfplus__mutations_rejected`, avec le motif du rejet : 32 129 `valeur_fonciere_nulle` et 5 288 `valeur_fonciere_non_positive`. Cette seconde branche **était à zéro** tant que le projet n'analysait que 2021-2025 : elle avait été écrite d'avance plutôt que déduite des données du jour, et l'ouverture aux années 2014-2020 l'a peuplée sans qu'une ligne de code change. Elles n'entrent pas dans `fct_mutations`, donc **pas non plus dans `n_ventes_total`**.

## Limites déclarées

Le jeu de données a des trous et le pipeline a un coût. Les deux sont écrits ici et rappelés dans l'application.

**Couverture de la source.** 97 départements sur 101. Manquent la Moselle (57), le Bas-Rhin (67) et le Haut-Rhin (68), sous *droit local* avec livre foncier et non couverts par les DVF, ainsi que Mayotte (976). C'est une lacune de la source, pas du pipeline, et elle se voit à l'œil nu sur la carte.
...
3. **Les actes multi-communes (180 390, 1,19 %) n'ont pas de commune.** Ils comptent au département et sont absents de la table commune - 39 553 d'entre eux dans le périmètre Maison/Appartement.
4. **La population des prix n'est pas celle du volume.** 37 383 ventes éligibles (0,38 %) sont écartées des médianes parce qu'elles ne sont pas des observations de marché : 36 052 sous 100 €/m² - prix symboliques, nue-propriété, cessions intrafamiliales -, 1 320 au-dessus de 40 000 €/m², 26 surfaces hors plage. Le cas d'école est mesurable : `58131 / Appartement / 2021` compte 29 ventes et **aucun prix utilisable** - dans certaines cellules le prix symbolique est la norme, et la colonne `n_ventes_prix_exclues` existe pour qu'elles se dénoncent elles-mêmes.
...
6. **871 cellules commune (0,19 %) ont du volume et aucun prix observable**, pour 963 ventes réelles que le projet ne peut pas tarifer.

7. **Le seuil de volume ne capture pas toute l'incertitude d'une variation.** Une cellule peut dépasser largement les 30 ventes et porter quand même une variation fragile. Mesuré par bootstrap (1 000 rééchantillonnages) sur les 194 paires département × type de la comparaison 2014-2025 : largeur médiane de l'intervalle à 95 % de **7,6 points**, mais **14 paires dépassent 20 points**. Le contraste tient dans un seul département : à Paris les **appartements** font +20,4 % avec un intervalle de [+19,8 ; +21,0] - 1,2 point de large, sur près de 25 000 ventes - et les **maisons** +24,1 % avec [+4,5 ; +41,8], 37 points de large sur 114 ventes. Même ville, même période, deux chiffres dont un seul est solide ; c'est `n_ventes_eligible` qui dit lequel. L'élargissement à douze années a resserré la plupart des intervalles - la médiane était de 6,3 points sur la seule fenêtre 2021-2025, mais sur des bases deux fois plus minces - tout en rendant comparables des écarts de onze ans.

À la maille commune, trois limites de plus :

- **86,95 % des cellules** sont sous le seuil de 30 : le classement montre 13,05 % de la table. Ce n'est pas un défaut, c'est ce qu'implique la maille commune sur douze ans de DVF ;
...
- **Paris, Lyon et Marseille n'existent pas comme communes dans les DVF** : la source les porte en **45 arrondissements**, traités tels quels, sans agrégation - agréger permettrait de sommer les comptages mais **pas** les médianes (le rapprochement croisé des deux marts le mesure : 2 325 cellules sur 2 328 diffèrent, jusqu'à 1 962 €/m²). Coût de cette fidélité : sur 991 cellules d'arrondissement, **272 (27,4 %) sont sous le seuil**, toutes `Maison` - aucune cellule `Appartement` sur 540 n'y tombe. « Le prix des maisons à Paris » n'est pas une question à laquelle ce jeu de données réponde avec robustesse.
...
- `assert_yoy_garde` - la variation annuelle est **absente** (`NULL`, et non zéro) partout où l'année précédente n'est pas adjacente, la fenêtre étant reconstruite par jointure, indépendamment du modèle. Sans ce garde-fou, un lag positionnel produirait 26 228 nombres plausibles et faux sur la table actuelle ;
- `assert_vefa_exclue` - l'exclusion du neuf est prouvée par le drapeau `vefa` de la source, une colonne **indépendante** de celle que le filtre utilise ;
